...
function dispatch(args) {
    ...
    try {
        switch (cmd) {
            ...
            case "convert_subevent_kind":
                convert_subevent_kind(args[1], args[2], args[3], args[4]);
                break;
            ...
            case "set_current_val":
                set_current_val(args[1], args[2]);
                break;

            case "set_current_val_vector3":
                set_current_val_vector3(args[1], args[2], args[3], args[4]);
                break;

            case "clear_current_vals":
                clear_current_vals();
                break;
...
function convert_subevent_kind(eventId, index, newKind, valueType) {
    eventId = requireExistingId(eventId);
    index = toInt(index, -1);
    newKind = String(newKind || "");
    valueType = String(valueType || "scalar");

    if (!validSubeventIndex(eventId, index)) return;

    var oldSub = model.events[eventId].subevents[index];
    var oldDelay = oldSub.delay_ms !== undefined ? oldSub.delay_ms : 0;

    if (newKind === "transport") {
        convert_subevent_to_transport_at_index(eventId, index, oldDelay);
        return;
    }

    if (newKind === "automation") {
        convert_subevent_to_automation_at_index(eventId, index, valueType, oldDelay);
        return;
    }

    errorOut("unsupported subevent kind conversion: " + newKind);
}

function convert_subevent_to_transport_at_index(eventId, index, delay_ms) {
    var sub = defaultTransportLoopSubevent();

    sub.target_time_ms = 0;
    sub.max_count = 1;
    sub.loop_id = eventId + "_jump_" + index;
    sub.delay_ms = toInt(delay_ms, 0);
    sub.enabled = false;

    model.events[eventId].subevents[index] = sub;

    outlet(0, "subevent_converted", eventId, index, "transport");
    emit_status("converted subevent to transport on " + eventId + "[" + index + "]");
}

function convert_subevent_to_automation_at_index(eventId, index, valueType, delay_ms) {
    var sub = defaultAutomationSubevent("set");
    ...
    if (valueType === "scalar") {
        ...
        sub.components = [];
        sub.value = null;
    } else if (valueType === "xyz") {
        sub.value_type = "vector3";
        sub.components = ["x", "y", "z"];
        sub.value = [0, 0, 0];
    } else if (valueType === "aer") {
        sub.value_type = "vector3";
        sub.components = ["azimuth", "elevation", "roll"];
        sub.value = [0, 0, 0];
    } else {
        errorOut("unsupported automation value type: " + valueType);
        return;
    }

    model.events[eventId].subevents[index] = sub;

    outlet(0, "subevent_converted", eventId, index, "automation_" + valueType);
    emit_status("converted subevent to automation " + valueType + " on " + eventId + "[" + index + "]");
}
...
function evaluateRelevantAutomation(rel, index, time_ms) {
    ...
    if (sub.mode === "interpolate") {
        ...
        var startValue = getNumericStartValue(startValueResult, sub.target);
...
function evaluateVector3Automation(rel, index, time_ms) {
    ...
    if (sub.mode === "interpolate") {
        ...
        var startValueResult = computeTargetStateFromRelevant(rel, startTime, index - 1);
        var startValue = getVector3StartValue(startValueResult, sub.target);
...
function getVector3StartValue(result, target) {
    var c;

    if (result && result.exists && result.value_type === "vector3" && result.value && result.value.length >= 3) {
        return [
    ...
    c = getCurrentValForTarget(target);

    if (c && c.value_type === "vector3" && c.value && c.value.length >= 3) {
        return [
            toFloat(c.value[0], 0),
            toFloat(c.value[1], 0),
            toFloat(c.value[2], 0)
...
function getNumericStartValue(result, target) {
    var c;

    if (result && result.exists && isNumberLike(result.value)) {
        return toFloat(result.value, 0);
    }

    c = getCurrentValForTarget(target);

    if (c && c.value_type === "scalar" && isNumberLike(c.value)) {
        return toFloat(c.value, 0);
    }
...
function normalizeLoadedModel(parsed) {
    ...
    if (!parsed.timeline.current_vals) parsed.timeline.current_vals = {};
...
function set_current_val(target, value) {
    ...
    if (!target) {
        errorOut("current value target required");
        return;
    }

    if (!model.timeline.current_vals) {
        model.timeline.current_vals = {};
    }

    model.timeline.current_vals[target] = {
        value_type: "scalar",
        value: parseAtom(value)
    };

    emit_status("set current value for " + target);
}

function set_current_val_vector3(target, x, y, z) {
    target = normalizeTargetKey(target);

    if (!target) {
        errorOut("current vector3 target required");
        return;
    }
    ...
    emit_status("set current vector3 value for " + target);
}

function clear_current_vals() {
    model.timeline.current_vals = {};
    emit_status("cleared current values");
}

function getCurrentValForTarget(target) {
    target = normalizeTargetKey(target);

    if (!model.timeline.current_vals) return null;
    if (!model.timeline.current_vals.hasOwnProperty(target)) return null;

    return model.timeline.current_vals[target];
}
