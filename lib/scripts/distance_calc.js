...
function setPlayerCount(num) {
  ...
  for (let i = 0; i < player_count; i++) {
    players.push(new player([0, 0, 0], [0, 0])); // initialize player with [x,y,z] position, and [azi, ele] rotations
  }
}
...
function player(pos, rot) {
  ...
  this.room = 0; // index of the currently encapsulating room with possession (-1 indicates no possesor)
  this.dist = Array(player_count).fill(0); // distances to each other player
  this.angles = Array(player_count); //relative angles to players close enough to hear
}

// All data for a player object
function getPlayer(num) {
  outlet(1, "player_" + num + " pos " + players[num].pos);
  outlet(1, "player_" + num + " rot " + players[num].rot);
  outlet(1, "player_" + num + " room " + players[num].room);
  outlet(1, "player_" + num + " dist " + players[num].dist);
  outlet(1, "player_" + num + " angles " + players[num].angles);
}
...
function getPlayerRot(num) {
  outlet(2, players[num].rot);
}

function getPlayerRoom(num) {
  outlet(1, players[num].room);
}

function getPlayerDist(num) {
  outlet(0, players[num].dist);
}

function getPlayerAngles(num) {
  outlet(2, players[num].angles + "");
}
...
/*
Convert a 3D point (x, y, z) into azimuth/elevation (degrees) under the system:
x = front, y = left, z = up
Returns { az, el } where:
- az is in [0..360)
- el is in [-90..90]
