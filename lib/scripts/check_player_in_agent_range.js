...
// list of players in range for each agent (nested two value array: boolean isWithinSensingRange, isWithinDimensionRange)
    ...
    for (let i = 0; i < agent_positions.length; i++) {
        for (let j = 0; j < player_positions.length; j++) {
            ...
            for (let i = 0; i < agent_positions.length; i++) {
                ...
                players_in_range[i] = [isWithinSensingRange, isWithinDimensionRange];
            }
        }

        // output players in range array
        outlet(0, players_in_range);
    }
}