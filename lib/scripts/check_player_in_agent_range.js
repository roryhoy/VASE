...
function debug_mode(mode) {
    debug = mode;
    outlet(2, "Debug mode set to: " + debug);
}
...
function agent_position(...args) {
    ...
    if (debug) {
        outlet(2, "Added agent position: " + agent_positions[agent_positions.length - 1]);
    }
}
...
function agent_dimension(...args) {
    ...
    if (debug) {
        outlet(2, "Added agent dimension: " + agent_dimensions[agent_dimensions.length - 1] + " and range: " + agent_ranges[agent_ranges.length - 1]);
    }
}
...
function agent_rotation(...args) {
    ...
    if (debug) {
        outlet(2, "Added agent rotation: " + agent_rotations[agent_rotations.length - 1]);
    }
}
...
function player_position(...args) {
    ...
    if (debug) {
        outlet(2, "Added player position: " + player_positions[player_positions.length - 1]);
    }
}
...
function bang() {
    ...
    for (let i = 0; i < agent_positions.length; i++) {
        for (let j = 0; j < player_positions.length; j++) {
            ...
            if (debug) {
                outlet(2, "Checking Agent " + i + " and Player " + j);
            }
            ...
            if (debug) {
                outlet(2, "Agent: " + i + ", Player: " + j + ", Distance Sq: " + distanceSquared + ", Ranged Sq: " + rangeSquared + ", Range Score: " + range_score);
            }
        }
    }
    // output players in range array
    for (let k = 0; k < players_in_range.length; k++) {
        outlet(0, k+1, players_in_range[k]);
    }

    // output player-agent distances    
    for (let m = 0; m < player_agent_distances.length; m++) {
        outlet(1, m+1, player_agent_distances[m]);
    }
}